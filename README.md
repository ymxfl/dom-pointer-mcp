...
- 🖼️ **粘贴参考图** —— 在 note panel 里 `Ctrl/Cmd+V` 粘贴任意外部截图作为参考图，可粘多张、显示缩略图，随选区一起发给 AI（用途在 note 里描述，例如"参考这张图的样式改选区"）
...
如果某个元素和周围元素挨得太近、不好用鼠标点中：选中任意一个后，用 **方向键** 把选择框移到目标上 —— `↑` 移到父级、`↓` 移到第一个子级、`←` `→` 在同级之间移动。方向键只调整 **最后选中** 的那个元素，其它已选元素不受影响（光标在输入框中时不拦截）。

要取消某个选中：再次 Option+Click 该元素，或者点 chip 上的 ×。Note panel 会一直保留，直到 **所有** 选中都被取消——防止你正在写的文本被误点丢掉。

如果 note panel 挡住了想选的其他元素，**按住面板空白处即可拖动**到任意位置（拖动后不再自动吸附到选中元素）。
