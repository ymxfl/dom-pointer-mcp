...
MCP Pointer 是一个 *本地* 工具，由 MCP Server 和 Chrome 扩展两部分组成：
...
1. 打开 [GitHub Releases](https://github.com/etsd-tech/mcp-pointer/releases)
2. 下载最新版的 `mcp-pointer-chrome-extension.zip`
3. 解压到本地任意目录
4. Chrome → 设置 → 扩展程序 → 开启 **开发者模式**
5. 点击"加载已解压的扩展程序"，选择解压目录
6. 工具栏出现 MCP Pointer 图标
7. **刷新已打开的网页** 让扩展生效
...
4. **Slash command?** —— 一个 y/N 同时套用于所有选中的 agent。
...
```bash
# 安装单个 agent（旧用法仍可用）
npx -y @mcp-pointer/server config claude       # 也支持 cursor / windsurf / codex / opencode / joycode
npx -y @mcp-pointer/server config claude --scope project

# 卸载（对称：清掉 MCP 注册、skill、斜杠命令）
npx -y @mcp-pointer/server config --uninstall            # 交互式卸载
npx -y @mcp-pointer/server config --uninstall claude     # 单个 agent，默认 user scope
...
> **可选：** 用 `npm install -g @mcp-pointer/server` 全局安装后可以直接用 `mcp-pointer` 取代 `npx -y @mcp-pointer/server`。

</details>

配置完成后，**重启 AI 工具** 让 MCP 连接生效。

> **🔄 已经在用 MCP Pointer？** 再跑一次 config 升级到 v1.0+ 的增量 MCP 注册逻辑——已有的 `~/.mcp.json` 等会被合并而不是覆盖。
...
AI 工具会在需要的时候用 `npx -y @mcp-pointer/server@latest start` 自动拉起 MCP server。
...
1. 确认 MCP server 已启动：`npx -y @mcp-pointer/server@latest start`
...
1. 配置后重启 AI 工具
2. 再跑一次 `npx -y @mcp-pointer/server config`，确认你的工具同时显示 MCP（和可选的斜杠命令）为 `installed`
3. 确认 server 在跑：`npx -y @mcp-pointer/server@latest start`
